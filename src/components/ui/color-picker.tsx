...
export type ColorFamily = keyof typeof TAILWIND_COLORS
export type ColorShade = keyof typeof TAILWIND_COLORS.slate

// Default shades to show in the compact grid (showing representative shades)
const DEFAULT_SHADES: ColorShade[] = [400, 500, 600]

// Color families grouped for better organization
        ...
        {COLOR_FAMILIES.map((family) => (
          <div key={family} className="space-y-1.5">
            ...
            <div className="flex gap-1">
              {Object.entries(TAILWIND_COLORS[family]).map(([shade, hex]) => (
                <button
                  key={`${family}-${shade}`}
                  type="button"
                  className={cn(
                    'h-6 w-6 rounded-md border border-border/50 transition-all hover:scale-110 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
                    value === hex && 'ring-2 ring-ring ring-offset-2'
                  )}
                ...
                >
                  {value === hex && (
                    <Check
                      className={cn(
                        'h-4 w-4 mx-auto',
                        parseInt(shade) >= 500 ? 'text-white' : 'text-gray-800'
                      )}
                    />
                  )}
                </button>
...
// Compact version showing fewer shades
...
export function CompactColorPicker({
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {shades.map((shade) => (
        <div key={shade} className="flex flex-wrap gap-1">
          {COLOR_FAMILIES.map((family) => {
...
export { TAILWIND_COLORS as tailwindColors }
